(function($,undefinded){
   ...
   $.cookie = function(name, value, options) {
    ...
    $.messages = function(options){
        var width = (typeof window.$analog_mobile=='boolean' && window.$analog_mobile)?"620px":"310px";
        var self = arguments.callee, msg, defaultOptions={"modal" : false, "width" : width,"closeOnEscape" : true};
        if(!self.uiDialog){
            self.uiDialog = $('<div title="提示:" style="height: 20px;font-size: 16px;"></div>').appendTo('body');
            self.uiDialog.dialog({autoOpen : false });
        }
        clearTimeout(self._timer);
        self.uiDialog.dialog("close");
    ...
    $.dialogEdit = function(){
        ...
        var self = arguments.callee, msg, defaultOptions={"modal" : false, "width" : width,"closeOnEscape" : true};
        if(!self.uiDialog){
	        self.uiDialog = $('<div title="输入附件名"></div>').appendTo('body');
	        self.uiDialog.dialog({autoOpen : false });
        }
        self.uiDialog.dialog("close");
		self.callback = $.Deferred();
		...
		options["buttons"] = {
			"确定" : function(){
				$("#edit_name").val($.trim($("#dialogEdit").val()));
				$( this ).dialog( "close" );
				self.callback.resolveWith(options);
			}
        };
        self.uiDialog.html('<input type="text" value="" id="dialogEdit" style="width:280px;">');
        self.uiDialog.dialog("option",options).dialog('open');
        if(self.callback){
        	return self.callback.promise();
        }else{
    ...
    /*
   	 * 弹框插件
   	 */  
    $.dialogRadio = function(typeIds){
    	var width = (typeof window.$analog_mobile=='boolean' && window.$analog_mobile)?"620px":"320px";
        var self = arguments.callee, msg, defaultOptions={"modal" : false, "width" : width,"closeOnEscape" : true};
        if(!self.uiDialog){
	        self.uiDialog = $('<div title="请选择"></div>').appendTo('body');
	        self.uiDialog.dialog({autoOpen : false });
        }
        self.uiDialog.dialog("close");
		self.callback = $.Deferred();
		var nameValue = {'541':'PLC物联网','540':'工业添翼','542':'智能制造','543':'其他生产线需求'};
		var options={};
		$.extend(options,defaultOptions);
		options["buttons"] = {
			"确定" : function(){
				var selectedTypeIds=[];
				var selectedName = [];
				$("#dialogRadio:checked").each(function(){selectedTypeIds[selectedTypeIds.length]=$(this).val()});
				for(var i=0;i<selectedTypeIds.length;i++){
					selectedName[selectedName.length]=nameValue[selectedTypeIds[i]];
				}
				$("#cluster_402").val(selectedName.join(","));
				$( this ).dialog( "close" );
				self.callback.resolveWith(options);
			}
        };
		...
		for(var i=0;i<typeIds.length;i++){
			...
			html += typeIds[i];
			html += '" id="dialogRadio" style="width:80px;">';
			html += nameValue[typeIds[i]];
			html += '</input><br/>';
		}
		self.uiDialog.html(html);
        self.uiDialog.dialog("option",options).dialog('open');
        if(self.callback){
        	return self.callback.promise();
        }else{
    ...
    /*
   	 * 追加备注弹框
   	 */  
    $.additionalRemarkDialog = function(){
    	var width = (typeof window.$analog_mobile=='boolean' && window.$analog_mobile)?"620px":"320px";
        var self = arguments.callee, msg, defaultOptions={"modal" : false, "width" : width,"closeOnEscape" : true};
        if(!self.uiDialog){
	        self.uiDialog = $('<div title="追加备注"></div>').appendTo('body');
	        self.uiDialog.dialog({autoOpen : false });
        }
        self.uiDialog.dialog("close");
		self.callback = $.Deferred();
        ...
        self.uiDialog.html('<textarea rows="3" cols="40" id="remark"></textarea>');
        self.uiDialog.dialog("option",options).dialog('open');
        if(self.callback){
        	return self.callback.promise();
        }else{
