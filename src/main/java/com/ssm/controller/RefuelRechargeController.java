...
@Controller
@RequestMapping("/refuelRecharge")
public class RefuelRechargeController extends BaseController{
	
	@Resource
	private RefuelRechargeService refuelRechargeService;
	...
	public Map<String, Object> showSinginInfo(Model model){
		...
		try {
			...
			reMap.put("code","0");
			List<RefuelRecharge> serList = refuelRechargeService.queryAllRefuelRecharge(); 
			reMap.put("length",serList.size());
			reMap.put("refuelRechargeList", serList);
			return reMap;
		} catch (Exception e) {
